...
async def procesar_productos(
    ...
    if not codigos:
        raise ValueError("Debes enviar al menos un cod_prod válido en 'productos'.")
    ...
    for cod_prod in codigos:
        ...
        try:
            detalle = await procesar_producto(cod_prod, estado_activo)
            _acumular_resultado_lote(resultado, detalle, cod_prod)
        except httpx.HTTPStatusError as e:
            resultado.errores += 1
            resultado.detalle_resultados.append(
                DetalleError(
                    cod_prod=cod_prod,
                    error=f"AVAX devolvió {e.response.status_code}: {e.response.text}",
                )
            )
